...
//   }
// }
var contains = function(string, checkString) {
  return string.indexOf(checkString) !== -1;
}
...
utils.livereloadSnippet = function livereloadSnippet(req, res, next) {
  ...
  var containsApi = contains(req.url, 'api');
  console.log(req.url, containsApi);
  console.log(path.extname( filepath ) !== '.html')
  console.log(path.extname( filepath ) !== '')

  if (path.extname( filepath ) !== '.html' && path.extname( filepath ) !== '' || containsApi)  {
    return next();
  }
